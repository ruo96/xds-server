...
func GetServices(sql string) []*Service {
	...
	for rows.Next() {
		var service = Service{}
		err := rows.Scan(&service.PartyId, &service.PyuServHost, &service.SpuServHost, &service.SpuProtoHost, &service.EnvoyPyuServPort, &service.EnvoySpuServPort, &service.EnvoySpuProtoPort)
		if err != nil {
