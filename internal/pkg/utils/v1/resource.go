...
import (
	"fmt"
	"github.com/envoyproxy/go-control-plane/pkg/cache/v3"    // 这两个是新增的
	"github.com/envoyproxy/go-control-plane/pkg/resource/v3" // 这两个是新增的
	"log"
	"time"
...
func makeTCPListener(service *Service) *listener.Listener {
	name := fmt.Sprintf("mpc-%s", service.PartyId)
	stdoutAccessLog, err := anypb.New(&access_loggers.StdoutAccessLog{})
	if err != nil {
		panic(err)
	}
	...
	return makeListener(service.PyuServHost, service.EnvoyPyuServPort, filterChains)
}
...
func makeClusterForService(service *Service) *cluster.Cluster {
	return &cluster.Cluster{
		Name:                          service.PyuServHost,
		ConnectTimeout:                durationpb.New(5 * time.Second),
		ClusterDiscoveryType:          &cluster.Cluster_Type{Type: cluster.Cluster_STRICT_DNS},
		LbPolicy:                      cluster.Cluster_ROUND_ROBIN,
		DnsLookupFamily:               cluster.Cluster_V4_ONLY,
		TypedExtensionProtocolOptions: makeTypedExtensionProtocolOptions(),
		LoadAssignment:                makeEndpoint(service.PyuServHost, service.PyuServHost, service.EnvoyPyuServPort),
	}
}
...
func makeRouteConfig(partyId string, type_ string, services []*Service) *route.RouteConfiguration {
	...
	for _, service := range services {
		...
		if service.PartyId != partyId {
			continue
		}
		virtualHosts = append(virtualHosts, &route.VirtualHost{
			Name:    service.PyuServHost,
			Domains: []string{service.PyuServHost},
			Routes: []*route.Route{{
